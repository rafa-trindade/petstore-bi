...
def petz_analysis():
    ...
    df_petz_filtrado = df_filtrado_tab1[df_filtrado_tab1["empresa"].str.lower() == "petz"]
    df_petz_filtrado_pop = df_filtrado_pop[df_filtrado_pop["empresa"].str.lower() == "petz"]
    df_outras = df_filtrado_tab1[df_filtrado_tab1["empresa"].str.lower() != "petz"]
    df_outras_pop = df_filtrado_pop[df_filtrado_pop["empresa"].str.lower() != "petz"]


    total_lojas_petz = len(df_petz_filtrado)
    total_lojas_outras = len(df_outras)

    total_lojas_petz_pop = len(df_petz_filtrado_pop)
    total_lojas_outras_pop = len(df_outras_pop)
    total_lojas_pop = total_lojas_petz_pop + total_lojas_outras_pop
    ...
    with col_m7:
        col_m7.success(f"Lojas Petz: {total_lojas_petz_pop}", icon=":material/store:")
        ...
        participacao_petz = participacao.get("petz", 0) * 100
        
        if not df_filtrado_tab1.empty:
            col_m7.success(f"Petz: {participacao_petz:.1f}% de participação no mercado")
        else:
    ...
    if df_filtrado_tab1.empty:
        ...
        df_petz_filtrado = df_petz_filtrado.copy()
        df_ibge_filtrado = df_ibge_filtrado.copy()

        df_filtrado_tab1["cidade"] = df_filtrado_tab1["cidade"].str.title().str.strip()
        df_petz_filtrado["cidade"] = df_petz_filtrado["cidade"].str.title().str.strip()
        ...
        df_okr3 = kpi.cidades_exclusivas_e_sem_empresas("petz", df_filtrado_tab1, df_ibge_filtrado)

        # Expansão por indice de saturacao
        df_okr4 = kpi.indice_saturacao_expansao("petz", df_filtrado_tab1, df_ibge_filtrado)

        # HHI
        df_okr5 = kpi.hhi_regiao(df_petz_filtrado, df_filtrado_tab1)
        ...
        okr_data = [
            {
                ...
                "Observação": "% Cidades > 250 mil Habitantes com Petz presente" if cidade_sel == "Todas" else "Selecione uma Região ou Estado"
            },
            {
                "OKR": "OKR2",
                ...
                "Meta": f"100%" if cidade_sel == "Todas" else "-",
                "Observação": f"% Capitais regionais com Petz presente" if cidade_sel == "Todas" else "Selecione uma Região ou Estado"
            },
            {
                "OKR": "OKR3",
                ...
                "Observação": f"Qtd. Cidades >100 mil Habitantes com presença exclusiva da Petz" if cidade_sel == "Todas" else "Selecione uma Região ou Estado"
            },
        ...
        if cidade_sel == "Todas":  
            ...
            with st.expander("GAP-OKR5"):
                ...
                |----------|----------------------------------------| --------------------------|
                | GAP-OKR5 | {df_okr5["cidades_prioritarias"]} | Cidades com alta concentração de mercado e baixa ou nula penetração da Petz |
                """, unsafe_allow_html=True)
