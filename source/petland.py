...
def petland_analysis():
    ...
    with col_1:
        st.success("**Petland** | Visão Geral", icon=":material/store:")
    with col_2:
    ...
    df_petland_filtrado = df_filtrado_tab1[df_filtrado_tab1["empresa"].str.lower() == "petland"]
    df_petland_filtrado_pop = df_filtrado_pop[df_filtrado_pop["empresa"].str.lower() == "petland"]
    df_outras = df_filtrado_tab1[df_filtrado_tab1["empresa"].str.lower() != "petland"]
    df_outras_pop = df_filtrado_pop[df_filtrado_pop["empresa"].str.lower() != "petland"]


    total_lojas_petland = len(df_petland_filtrado)
    total_lojas_outras = len(df_outras)

    total_lojas_petland_pop = len(df_petland_filtrado_pop)
    total_lojas_outras_pop = len(df_outras_pop)
    total_lojas_pop = total_lojas_petland_pop + total_lojas_outras_pop
    ...
    with col_m7:
        col_m7.success(f"Lojas Petland: {total_lojas_petland_pop}", icon=":material/store:")
        ...
        participacao_petland = participacao.get("petland", 0) * 100
        
        if not df_filtrado_tab1.empty:
            col_m7.success(f"Petland: {participacao_petland:.1f}% de participação no mercado")
        else:
    ...
    if df_filtrado_tab1.empty:
        ...
        df_petland_filtrado = df_petland_filtrado.copy()
        df_ibge_filtrado = df_ibge_filtrado.copy()

        df_filtrado_tab1["cidade"] = df_filtrado_tab1["cidade"].str.title().str.strip()
        df_petland_filtrado["cidade"] = df_petland_filtrado["cidade"].str.title().str.strip()
        ...
        df_okr1 = kpi.popopulacao_250K(df_petland_filtrado, df_ibge_filtrado)
        
        # Capitais
        df_okr2 = kpi.capitais(df_petland_filtrado, df_capitais_filtrado)

        # Exclusivida em cidades > 100 mil hab
        df_okr3 = kpi.cidades_exclusivas_e_sem_empresas("petland", df_filtrado_tab1, df_ibge_filtrado)

        # Expansão por indice de saturacao
        df_okr4 = kpi.indice_saturacao_expansao("petland", df_filtrado_tab1, df_ibge_filtrado)

        # HHI
        df_okr5 = kpi.hhi_regiao(df_petland_filtrado, df_filtrado_tab1)
        ...
        okr_data = [
            {
                ...
                "Observação": "% Cidades > 250 mil Habitantes com Petland presente" if cidade_sel == "Todas" else "Selecione uma Região ou Estado"
            },
            {
                "OKR": "OKR2",
                ...
                "Meta": f"100%" if cidade_sel == "Todas" else "-",
                "Observação": f"% Capitais regionais com Petland presente" if cidade_sel == "Todas" else "Selecione uma Região ou Estado"
            },
            {
                "OKR": "OKR3",
                "Indicador": "Cidades > 100 mil Habitantes exclusivas Petland",
                "Filtro": f"{filtro}",
                "Atual": f"{df_okr3['indice']}" if cidade_sel == "Todas" else "-",
                "Meta": f"+10%" if cidade_sel == "Todas" else "-",
                "Observação": f"Qtd. Cidades >100 mil Habitantes com presença exclusiva da Petland" if cidade_sel == "Todas" else "Selecione uma Região ou Estado"
            },
        ...
        if cidade_sel == "Todas":     
            with st.expander("GAP-OKR1"):
                ...
                |----------|----------------------------------------|--------------------------|
                | GAP-OKR1 | {df_okr1["cidades_ausentes"]} |Cidades >250 mil habitantes sem presença da Petland |
        ...
        if cidade_sel == "Todas":  
            ...
            with st.expander("GAP-OKR2"):
                ...
                |----------|----------------------------------------| --------------------------|
                | GAP-OKR2 | {df_okr2["cidades_ausentes"]} | Capitais Regionais sem presença da Petland |
                """, unsafe_allow_html=True)
            ...
            with st.expander("GAP-OKR5"):
                ...
                |----------|----------------------------------------| --------------------------|
                | GAP-OKR5 | {df_okr5["cidades_prioritarias"]} | Cidades com alta concentração de mercado e baixa ou nula penetração da Petland |
                """, unsafe_allow_html=True)
