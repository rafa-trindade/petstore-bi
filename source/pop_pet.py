...
def load_data():
    ...
    df = df[df["empresa"].str.lower() == "pop pet"]
    df = df.dropna(subset=["latitude", "longitude"])
    return df


def pop_pet_analysis():

    df_pop_pet = load_data()
    ...
    with tab1:
        ...
        if regiao_sel != "Todas":
            if regiao_sel:
                ...
                df_regiao = df_pop_pet[df_pop_pet["estado"].isin(estados_da_regiao)]
