...
def cobasi_analysis():
    ...
    df_cobasi_filtrado = df_filtrado_tab1[df_filtrado_tab1["empresa"].str.lower() == "cobasi"]
    df_cobasi_filtrado_pop = df_filtrado_pop[df_filtrado_pop["empresa"].str.lower() == "cobasi"]
    df_outras = df_filtrado_tab1[df_filtrado_tab1["empresa"].str.lower() != "cobasi"]
    df_outras_pop = df_filtrado_pop[df_filtrado_pop["empresa"].str.lower() != "cobasi"]


    total_lojas_cobasi = len(df_cobasi_filtrado)
    total_lojas_outras = len(df_outras)

    total_lojas_cobasi_pop = len(df_cobasi_filtrado_pop)
    total_lojas_outras_pop = len(df_outras_pop)
    total_lojas_pop = total_lojas_cobasi_pop + total_lojas_outras_pop
    ...
    with col_m7:
        col_m7.success(f"Lojas Cobasi: {total_lojas_cobasi_pop}", icon=":material/store:")
    ...
    if df_filtrado_tab1.empty:
        ...
        df_okr3 = kpi.cidades_exclusivas_e_sem_empresas("cobasi", df_filtrado_tab1, df_ibge_filtrado)

        # Expansão por indice de saturacao
        df_okr4 = kpi.indice_saturacao_expansao("cobasi", df_filtrado_tab1, df_ibge_filtrado)

        # HHI
        df_okr5 = kpi.hhi_regiao(df_cobasi_filtrado, df_filtrado_tab1)
        ...
        okr_data = [
            {
                ...
                "Observação": "% Cidades > 250 mil Habitantes com Cobasi presente" if cidade_sel == "Todas" else "Selecione uma Região ou Estado"
            },
            {
                "OKR": "OKR2",
                ...
                "Meta": f"100%" if cidade_sel == "Todas" else "-",
                "Observação": f"% Capitais regionais com Cobasi presente" if cidade_sel == "Todas" else "Selecione uma Região ou Estado"
            },
            {
                "OKR": "OKR3",
                ...
                "Observação": f"Qtd. Cidades >100 mil Habitantes com presença exclusiva da Cobasi" if cidade_sel == "Todas" else "Selecione uma Região ou Estado"
            },
        ...
        if cidade_sel == "Todas":  
            ...
            with st.expander("GAP-OKR5"):
                ...
                |----------|----------------------------------------| --------------------------|
                | GAP-OKR5 | {df_okr5["cidades_prioritarias"]} | Cidades com alta concentração de mercado e baixa ou nula penetração da Cobasi |
                """, unsafe_allow_html=True)
